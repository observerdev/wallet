$(document).ready(function() {
	...
	var explorer_tx = "https://explorer.wagerr.com/#/tx/"
	var explorer_addr = "https://explorer.obsr.org/#/address/"
	var explorer_block = "https://explorer.wagerr.com/#/block/"
	...
	$("#walletToLegacy").click(function(){
		...
		$("#walletSegwitp2sh")[0].checked = false;
		$("#walletSegwitBech32")[0].checked = false;	
		$("#openBtn").click();
	});
	...
	$("#walletConfirmSend").click(function(){
		...
		tx.addUnspent($("#walletAddress").html(), function(data){
			...
			if(dvalue>=total){
				...
				$("#walletSendConfirmStatus").removeClass("hidden").addClass('alert-danger').html("You have a confirmed balance of "+dvalue+" OBSR unable to send "+total+" OBSR").fadeOut().fadeIn();
				thisbtn.attr('disabled',false);
			}
	...
	function walletBalance(){
		...
		coinjs.addressBalance($("#walletAddress").html(),function(data){
			if($(data).find("result").text()==1){
				...
				$("#walletBalance").html(v+" OBSR").attr('rel',v).fadeOut().fadeIn();
			} else {
				$("#walletBalance").html("0.00 OBSR").attr('rel',v).fadeOut().fadeIn();
			}
	...
	function rawSubmitChainso_BitcoinMainnet(thisbtn){ 
		...
		$.ajax ({
			type: "POST",
			url: "https://chain.so/api/v2/send_tx/OBSR/",
	...
	function rawSubmitblockcypher_BitcoinMainnet(thisbtn){ 
		...
		$.ajax ({
			type: "POST",
			url: "https://api.blockcypher.com/v1/OBSR/main/txs/push",
